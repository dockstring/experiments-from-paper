...
from rdkit.Chem import Crippen, QED as qed_module
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path", type=str, required=True, help="Path of tsv file to output."
    )
    ...
    df["logP"] = [
        Crippen.MolLogP(Chem.MolFromSmiles(s)) for s in tqdm(smiles, desc="logP")
    ]
    df["QED"] = [
        qed_module.qed(Chem.MolFromSmiles(s)) for s in tqdm(smiles, desc="QED")
    ]
