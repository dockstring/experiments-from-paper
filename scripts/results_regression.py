...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metric", type=str, default="R2", help="Which metric to plot."
    )
    parser.add_argument(
        "--std",
        action="store_true",
    ...
    # Dataframe columns
    all_methods = list(
        set(method_name for r in protein_res_dict.values() for method_name in r.keys())
    )
    all_methods_renamed = [
        NAME_REMAP[name] if name in NAME_REMAP else name for name in all_methods
    ]
    reverse_method_name_map = {
        rename: orig_name for rename, orig_name in zip(all_methods_renamed, all_methods)
    }
    all_methods_renamed_sorted = [
        NAME_REMAP[n] for n in PREFFERED_ORDER if n in all_methods
    ]
    all_methods_renamed_sorted += sorted(
        [n for n in all_methods_renamed if n not in all_methods_renamed_sorted]
    )
    ...
    if args.latex:
        ...
        for row_name, row in df_latex.iterrows():
            ...
            df_latex.loc[row_name][
                df_latex.columns[max_idx]
            ] = "\\textbf{{{:s}}}".format(val_orig)
